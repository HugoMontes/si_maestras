<?php
class Formador_inscripcion extends CI_Controller{
    public function __construct()
    {
        parent::__construct();  
        
        existe_sesion_inscripcion();
    ...
    public function index()
	{
        ...
        $pagina_formador_inscripcion = $this->pagina_predisenada_model->get_values('metadescripcion, metapalabras_clave, id, hits,estado', array('alias'=>'inscripciones-formadores-score-bolivia'));
        if($pagina_formador_inscripcion->estado == PUBLICADO){
           $this->pagina_predisenada_model->update(array('hits'=>$pagina_formador_inscripcion->hits + 1),$pagina_formador_inscripcion->id);  
           $data['titulo'] = $this->lang->line('score_sitio_inscripcion_formador');
           $data['metadescripcion'] = $pagina_formador_inscripcion->metadescripcion;
           $data['metapalabras_clave'] = $pagina_formador_inscripcion->metapalabras_clave;
           ...
           $postulacion_formador = $this->session->userdata('postulacion_formador');
           $postulacion_formador = (object)$postulacion_formador;
           $data['postulante'] = $this->postulacion_formador_model->get($postulacion_formador->id); 
           
           // mas datos pendientes
           
           $data['ciudades_inscripcion'] = $ciudades_inscripcion;   
                   
           $this->load->view('frontend/formador_inscripcion',$data);        
        }elseif($pagina_formador_inscripcion->estado == DESPUBLICADO)
        {
