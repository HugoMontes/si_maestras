...
class CI_Page
{
    ...
    var $stats_title    = 'resultados'; // will show the title for the page stats
    var $view_all_link  = true; // will show the view all link
    var $view_all_text  = 'Ver todo'; // text for $view_all_link
    ...
    public function create_links()
    {
        if ($this->total_rows > $this->rows_per_page) {
            ...
            if ($this->cur_page > 1) {
                ...
                $prev = $this->create_link('Anterior', $page, true, $this->prev_class);
            } else {
                if ($this->show_prev) {
                    $prev = $this->create_link('Anterior', $page, true, $this->prev_class);
                }
            ...
            if ($this->cur_page < $this->max_pages) {
                $page = $this->cur_page + 1;
                $next = $this->create_link('Siguiente', $page, true, $this->next_class);
            } else {
                if ($this->show_next) {
                    $prev = $this->create_link('Siguiente', $page, true, $this->next_class);
                }
            }
    ...
    public function page_stats()
    {
        if ($this->total_rows == 0) {
            ...
            echo number_format($start, 0) .
                 " a " . number_format($end, 0) .
                 " (de " . number_format($this->total_rows, 0) .
                 " " .
                 $this->stats_title .
                 ")";
        }
